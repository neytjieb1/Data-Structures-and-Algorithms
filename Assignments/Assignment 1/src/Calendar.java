...
public class Calendar {
	...
	public void Insert(int day, String month, String description, String duration, int priority) {
		//Create node
		Item newNode = new Item(description, duration, priority, day, month.toLowerCase());

		//Month is empty
		if (rootMonth[newNode.getMonth()]==null) {
			//Both empty
			if (rootDay[day - 1] == null) {
				rootMonth[newNode.getMonth()] = newNode;
				rootDay[day - 1] = newNode;
				return;
			}
			//One item
			else if (rootDay[day-1].right == null) {
				if (rootDay[day-1].getMonth() < month) {

				}
				else if (rootDay[day-1].getMonth().equals(month)) {

				}
				else if (rootDay[day-1].getMonth() > month) {
